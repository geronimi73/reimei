...
class ImageNet96Dataset(torch.utils.data.Dataset):
    def __init__(self, hf_ds, bs, device, dtype, col_label="label", col_latent="latent"):
        ...
        self.siglip_model = SiglipModel.from_pretrained(SIGLIP_HF_NAME, cache_dir=f"{MODELS_DIR_BASE}/siglip").to(device, dtype)
        self.siglip_processor = SiglipProcessor.from_pretrained(SIGLIP_HF_NAME, cache_dir=f"{MODELS_DIR_BASE}/siglip")
        self.prompt_len = 50
        self.device = device
        self.dtype = dtype
        
        self.sampler = RandomSampler(hf_ds)
        self.dataloader = DataLoader(
            hf_ds, sampler=self.sampler, collate_fn=self.collate, batch_size=bs, num_workers=1, prefetch_factor=2
        )
    ...
    def __iter__(self):
        while True:
            for labels, latents in self.dataloader:
                ...
                yield {"caption": labels, "ae_latent": latents, "siglip_emb": emb, "siglip_vec": vec}
    
    @torch.no_grad
    def encode_siglip(self, captions):
        # Encode the captions
        s_tokens = self.siglip_processor.tokenizer(captions, padding='longest', truncation=True, return_tensors="pt", max_length=MAX_CAPTION_LEN).to(self.device)
        siglip_outputs = self.siglip_model.text_model(**s_tokens, output_hidden_states=True)
